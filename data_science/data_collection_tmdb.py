...
for i in range(1, 501): 
    response = requests.get(
        f"https://api.themoviedb.org/3/movie/popular?api_key={APIKEY}&language=en-US&page={i}"
    )
...
for i in ids:
    response = requests.get(
        f"https://api.themoviedb.org/3/movie/{i}?api_key={APIKEY}&language=ko-KR"
    )
    temp1 = pd.DataFrame.from_dict(response.json(), orient="index")
...
kor_mv.rename(
    columns={
        "title": "kor_title",
        "overview": "kor_overview",
        "poster_path": "kor_image_path",
    },
    inplace=True,
)
...
tmdb_ott_t.rename(
    columns={"tv_rating": "content_rating", "cast": "actor", "duration": "runtime"},
    inplace=True,
)
...
pop_mv_details.rename(
    columns={
        "original_title_x": "title",
        "overview_x": "overview",
        "poster_path_x": "image_path",
        "vote_average_y": "rating",
        "vote_count_y": "vote_count",
    },
    inplace=True,
)
pop_mv_details = pop_mv_details[
    ["id", "title", "rating", "image_path", "overview", "vote_count"]
]
...
for i in movie_ids:
    response = requests.get(
        f"https://api.themoviedb.org/3/movie/{i}/reviews?api_key={APIKEY}"
    )
...
for i in tv_ids:
    response = requests.get(
        f"https://api.themoviedb.org/3/tv/{i}/reviews?api_key={APIKEY}"
    )
    temp1_rev = pd.DataFrame.from_dict(response.json()["results"])
    temp1_rev["content_id"] = i
    temp2_rev.append(temp1_rev)
